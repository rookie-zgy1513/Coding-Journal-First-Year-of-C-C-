...
ptnode Nwords;
ptnode stopwords_tire;
ptnode weight[100001],allweight[100001],stopweight[100001];					//储存weight,这样树不用再遍历一遍
...
ptnode talloc();
ptnode wordTire0(ptnode root,char *w);
ptnode wordTire1(ptnode root,char *w);
ptnode wordTire2(ptnode root,char *w);
...
int main(int argc,char *argv[])
{
	Nwords=talloc();
	int a,b,N=0,M=0;
	int length_N=strlen(argv[1]),length_M=strlen(argv[2]);
	for(a=0;a<length_N;a++)
	{
		N=N*10+argv[1][a]-'0';
	}
	for(b=0;b<length_M;b++)
	{
		M=M*10+argv[2][b]-'0';
	}
	char word[100];
//建立停用词树
	...
	ptnode stopwords_tire=talloc();
	while(getword(fp_stopwords,word)!=-1)
	{
		stopwords_tire=wordTire2(stopwords_tire,word);
	}
	fclose(fp_stopwords); 
	//停用词树建立完成
//先将N行，M列hash值提取
...
//打开已获取网页，通过字典树确定前N个单词
	...
	//对当前网页和全体网页建立相应特征字典树
		...
		while(getword(fp_article,word)!=-1)
		{
			...
			if(!(*mem=='\0'&&q->isword==1))
			{
				root=wordTire1(root,word);
				Nwords=wordTire0(Nwords,word);			//全体网页字典树建立中
			}
		}
...
//for(int z=0;z<Web_count;z++)
	...
	//对当前网页建立相应特征字典树
		...
		{
			int i=0;
			ptnode q=Web_new[Web_new_count].root;
			for(;mem[m]->word[i]!='\0';i++)
			{
				if(q->ptr[mem[m]->word[i]-'a']==NULL)
				{
		...
		//��ʱWeb_new[Web_new_count]��ָ���ѽ������ ,ע�⣺ֻ���ַ����飬�����ַ��������� 	
		Web_new_count++;
		if(!feof(fp_sample))
			sym=fgetc(fp_sample);
	}
...
{
	int i;
	ptnode p=(ptnode)malloc(sizeof(tnode));
...
{
	ptnode *e1=(ptnode*)E1,*e2=(ptnode*)E2;
