...
#define MaxSize 250
//思路：目前我只会计算后缀表达式，对于中缀表达式，考虑采用中缀转后缀再计算中缀的做法
...
void MiddleToSuffix(char str[]) {
	...
	for (int i = 0; i < strlen(str); i++) {	//依次扫描字符串
		...
		/* 若为界限符，左括号则压入运算符栈，右括号则弹出运算符栈的运算符直到遇到左括号并输            　　　　　　　
　　　　　出显示，后缀表达式是不显示括号的；同时每次弹出一个运算符，则依次弹出操作数栈的两个操　　　　　　　　 
          作数分别作为右操作数和左操作数计算结果并压入操作数栈*/
		if (str[i] == '(') {
			StrS.op[++StrS.top] = str[i];
		}//左括号则压入运算符栈
		else if (str[i] == ')') {
			while (StrS.op[StrS.top] != '(') {
				Calculate(&StrS, &NumS);
			}//右括号则弹出运算符栈的运算符直到遇到左括号并输出显示，同时每次弹出一个运算符，则依次弹出操作数栈的两个操 作数分别作为右操作数和左操作数计算结果并压入操作数栈
			StrS.top--;	//将左括号出栈
		}
		// 若为运算符，弹出运算符栈中优先级更大和同级的运算符直到栈空或左括号，同时输出弹出 的运算符以及计算结果，最后将该运算符入栈
		else if (str[i] == '+' || str[i] == '-') {	//由于加法和减法优先级最低，故运算符栈栈顶只要是运算符都会一直出栈 
			while (StrS.top != -1 && StrS.op[StrS.top] != '(') {
				Calculate(&StrS, &NumS);
			}
			StrS.op[++StrS.top] = str[i];
		}
		else if (str[i] == '*' || str[i] == '/') {	//由于乘法和除法优先级最高，故只能运算符栈顶是乘法或除法才可以持续弹出
                                                       
			while (StrS.top != -1 && StrS.op[StrS.top] != '(' && (StrS.op[StrS.top] == '*' || StrS.op[StrS.top] == '/')) {
				Calculate(&StrS, &NumS);
			}
			StrS.op[++StrS.top] = str[i];
		}
